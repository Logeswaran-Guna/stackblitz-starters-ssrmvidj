-- A parent and a tutor account are no longer mutually exclusive on the
-- same profile: a parent can also register as a tutor, and a tutor can
-- also submit requirements as a parent. profiles.role stays as the
-- account's original/primary role (used for login routing and the
-- Header's default dashboard link) — this migration just stops the RPCs
-- underneath both flows from hard-blocking the "other" role. The data
-- model already supported this (teacher_profiles.user_id and
-- requirements.parent_id are independent foreign keys to profiles.id,
-- never gated by profiles.role) — only the role checks needed relaxing.
--
-- Known limitation, not fixed here: _recompute_status() (0012) still
-- looks only at the profile's PRIMARY role when deciding Active/Idle, so
-- a dual-role account's activity on their secondary side isn't counted
-- toward their status yet. Registering and submitting as both roles
-- works correctly; status computation for the secondary role is a
-- separate follow-up.

-- === become a tutor, now allowed for a PARENT account too ==================
...
-- === "do I already have a tutor application?" check, now allowed for a
-- === PARENT account too — used by become-a-tutor to decide whether to
-- === show the form or a pending/active status message. Returns zero
-- === rows (not an error) if the caller has no teacher_profiles row yet.
...
-- === a tutor's own submitted requirements (as a parent), now allowed
-- === for a TEACHER account too ==============================================
...
-- === languages step of the tutor application, now allowed for a PARENT
-- === account too ============================================================
...
-- === KYC document upload step of the tutor application, now allowed for
-- === a PARENT account too ===================================================
